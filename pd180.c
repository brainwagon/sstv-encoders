 ...
 * |_| |___/|_\___/\__/ 
 *  
 * An implementation of a PD180 encoder
...
#define WIDTH 		640
#define HEIGHT		(480+16)
#define HALFHEIGHT	(HEIGHT/2)

typedef float Image[HEIGHT][WIDTH][3] ;
typedef float HalfImage[HALFHEIGHT][WIDTH][3] ;
...
BufferFloat(float f)
{
    sampbuf[nsamps++] = /* filtersample(f) */ f ;
...
{
    int i, j ;
    int samp = 183.040 * SAMPLERATE / 1000 ;

    pulse(1200, 20.0) ;		/* sync pulse */
    pulse(1500, 2.080) ;	/* sync porch */
...
main(int argc, char *argv[])
{
	...
	for (i=0; i<WIDTH; i++) {
	    for (c=0; c<3; c++)
		halfimage[j][i][c] = (image[2*j][i][c]+image[2*j+1][i][c]) / 2.;
	}
    }
    ...
    blank(500.0) ;
    Vis(96) ;			/* Robot 72 is VIS mode 12 */
